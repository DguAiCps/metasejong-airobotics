...
def genetic_algorithm(worker_num, min_delta = 1e-6, patience = 15):
...
def visit_order(data, robot_pos) -> list[tuple[str, tuple[float, float, float]]]:
    items = [(key, tup)
    for key, tup_list in data.items()
            for tup in tup_list]

    # TODO: 실제 로봇 위치 구하는 함수로 변경
    ...
    for i in range(num_objects-1):
        p, q = object_coords[order[i]], object_coords[order[i+1]]
